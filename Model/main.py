...
from dataset_aug import Imgdataset, normalize_augment
...
class Handler(object):
    def __init__(self, args):
        ...
        self.maskpath = self.args.train.maskpath

        self.model = VCS_UF(args.model.num_stage, 1, args.model.color, args.flex.cr_model, 
                            args.model.width, args.model.num_blocks, args.model.width_ratio,
                            args.model.shortcut, args.model.Mask_info, args.model.CR_info,
                            args.model.losstype, args.model.num_loss, args.model.weight_loss).to(self.device)

        self.model.stat_params(input_size=[args.train.patch_size, args.train.patch_size, args.flex.cr_model], 
                                    device=self.device, 
                                    log=self.logger)
    ...
    def train(self):
        ...
        train_dataloader = DataLoader(
                dataset=Imgdataset(crop_size=self.args.train.patch_size, color=self.args.model.color, length=self.args.flex.cr_train[0], skip=self.args.flex.cr_train[0], root_path=self.args.train.datapath), 
        ...
        for e in range(self.start_epoch, self.args.train.epochs):
            ...
            for iteration, data in enumerate(train_dataloader):
                ...
                rand_cs = random.randint(0, len(self.args.flex.cr_train)-1)
                self.cr_now = self.args.flex.cr_train[rand_cs]
                cc = random.randint(0, data.shape[1]-self.cr_now)
                data = normalize_augment(data, self.cuda, self.cr_now, cc)
                ...
                if self.args.useAMP:
                    with torch.cuda.amp.autocast():
                        Loss = self.model(data, mask, rgb2bayer)
    ...
    def valid(self):
        ...
        for i in range(len(self.valid_list)):
            ...
            time1 = time.time()
            with torch.no_grad():
                out_pic = self.model.forward_main(meas, mask, rgb2bayer)[-1]
    ...
    def make_dirs(self):
        ...
        os.system('cp Model/configs/{} {}/configs.yml'.format(self.args.config, self.ckpt_dir))
