...
class Handler(object):
    def __init__(self, args):
        ...
        self.mask = torch.from_numpy(np.float32(sio.loadmat(args.train.maskpath)['mask'])).to(self.device).permute(2,0,1).unsqueeze(0)
        # [1, cr, H, W] gpu

        if args.stat:
            y = torch.randn((1, 1, 256, 256)).to(self.device)
            phi = torch.randn((1, 8, 256, 256)).to(self.device)
            flops, params = profile(self.model, inputs=(y, phi), verbose=False)
            self.logger.info('[NET_SIZE] FLOPs:{:>6.2f}G | Params:{:>5.2f}M'.format(flops/1e9, params/1e6))
    ...
    def make_dirs(self):
        ...
        self.ckpt_dir = 'Checkpoints/{}/{}_{}'.format(self.args.model.name, self.date_time, self.args.model.tag)
